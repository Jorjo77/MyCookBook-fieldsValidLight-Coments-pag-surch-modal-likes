...
export async function getAllCommentsByRecipe(recipeId) {
    return await api.get(host + `/data/comments?where=recipeId%3D%22${recipeId}%22`);
}
...
export async function getAllLikes(recipeId) {
    return await api.get(host + `/data/likes?where=recipeId%3D%22${recipeId}%22&distinct=_ownerId&count`);
}

//get like for an Recipe from a specific user
export async function getLikeForRecipeFromSpecUser(recipeId, userId) {
    return await api.get(host + `/data/likes?where=recipeId%3D%22${recipeId}%22%20and%20_ownerId%3D%22${userId}%22&count`);
}